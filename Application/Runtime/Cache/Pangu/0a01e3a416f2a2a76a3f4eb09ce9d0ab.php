  ...
  <a class="btn btn-primary radius" onclick="admin_add('添加','<?php echo U('Pangu/Index/edit');?>?table=news&type=<?php echo ($type); ?>','800','800')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
          ...
          <th width="80">最大人数</th>
          <th width="80">最高价格</th>
          <th width="80">最低砍价金额</th>
          <th width="80">最高砍价金额</th>
          <th width="80">操作</th>
				</tr>
			</thead>
			<tbody>
          ...
          <td><?php echo ($vo["max_peo"]); ?></td>
          <td><?php echo ($vo["cost_price"]); ?></td>
          <td><?php echo ($vo["rand_a"]); ?></td>
          <td><?php echo ($vo["rand_b"]); ?></td>
					<td class="f-14 td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="admin_edit('编辑','<?php echo U('Index/edit');?>?table=kanjia&type=<?php echo ($type); ?>&id=<?php echo ($vo["id"]); ?>','1000','800')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
					</td>
...
$(function(){
  $(".del_all").click(function(){
      ...
      $(".hidden-xs").each(function(){
          ...
          $.get("<?php echo U('Index/do_del_all');?>?table=tuan&ids="+ids,function(data,s){
  ...
  $(".w_del").click(function(){
    ...
    $.post("<?php echo U('Index/do_del');?>",{id:id,table:"tuan"},function(data){
      if (data==1) {
      alert("删除成功")     
